function filterFruits() {
    ...
    const items = document.querySelectorAll('.Fruits-list .item'); // Targeting items inside vegetable-list

    items.forEach(item => {
        const FruitsName = item.querySelector('h1').textContent.toLowerCase(); // Get the vegetable name from the h1 tag
        if (FruitsName.indexOf(filter) > -1) {
